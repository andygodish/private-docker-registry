...
do_download() {
  ...
  if [ "${SKIP_IMAGES_DL}" = "false" ]; then
    ...
    CHECKSUM_ACTUAL=$(sha256sum "rke2.linux-amd64.tar.gz" | awk '{print $1}');
    CHECKSUM_EXPECTED=$(grep "rke2.linux-amd64.tar.gz" "sha256sum-amd64.txt" | awk '{print $1}');
    if [ "${CHECKSUM_EXPECTED}" != "${CHECKSUM_ACTUAL}" ]; then echo "FATAL: download sha256 does not match"; exit 1; fi
...
workdir=rke-government-deps-$(date +"%y-%m-%d-%-k-%M-%S");
mkdir $workdir;
cd $workdir;
yum install -y pigz yum-utils createrepo unzip epel-release;
